...
module.exports = {
  ...
  async run(bot, interaction) {
    ...
    if(member == null){
      ...
      require(`../events/.log.js`).run(bot, `[GAME-ENTER] : **${interaction.member.user.username}**`)
    } else {
      interaction.reply({content: 'You have already been granted your starting points.', ephemeral: true})
      require(`../events/.log.js`).run(bot, `[GAME-ENTER-ERROR] : **${interaction.member.user.username}**`)
    }
  }
